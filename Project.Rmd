...
ggplot(rs_246, aes(x = Year, y = NumericValue)) +
  geom_point(aes(color = `Road User Type`), alpha = 0.5) +
  geom_smooth(method = "lm", aes(color = `Road User Type`), se = FALSE) +
  labs(title = "Rate Over Time by Road User Type",
       x = "Year",
       y = "NumericValue") +
  theme_minimal()
...
- **Residuals vs Fitted Plot**: The absence of distinct patterns suggests that the model's assumptions of linearity and homoscedasticity are reasonable. However, the constant spread across fitted values needs to be observed for any potential heteroscedasticity.

- **Normal Q-Q Plot**: This plot is used to assess the normality of residuals. Deviations from the reference line at the extremes could indicate potential outliers or heavy tails in the distribution of residuals.

- **Scale-Location Plot**: A horizontal scatter suggests that the variances are equal across the range of fitted values, which is desirable in a good model.

- **Residuals vs Leverage Plot**: Points that stand out far from the rest of the data points might have high leverage and could unduly influence the model. Cook's distance lines help to identify influential cases that may merit further investigation.
