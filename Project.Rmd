...
ggplot(combined_summary, aes(x = Year, y = Value)) +
  geom_line() +
...
# Plotting RS_196
ggplot(rs_196_agg, aes(x = Year, y = Average_Deaths, color = Sex)) +
  geom_line() +
  geom_point() +
...
# Plotting RS_198
ggplot(rs_198_agg, aes(x = Year, y = Average_Rate, color = Sex)) +
  geom_line() +
  geom_point() +
...
ggplot(rs_198, aes(x = Year, y = Rate, color = Sex, group = Sex)) +
  geom_line(alpha = 0.5) +
...
ggplot(rs_196, aes(x = Year, y = Rate, color = Sex, group = Sex)) +
  geom_line(alpha = 0.5) +
  geom_point(size = 2, alpha = 0.5) +
  geom_errorbar(aes(ymin = Low, ymax = High), width = 0.4, linewidth = 0.5, alpha = 0.8) +
...
ggplot(region_data_196, aes(x = Year, y = Rate, color = `Geographic Value`)) +
  geom_line() +
  geom_point() +
...
ggplot(region_data_198, aes(x = Year, y = Rate, color = `Geographic Value`)) +
  geom_line() +
  geom_point() +
  facet_wrap(~`Geographic Value`, scales = "free_y") +
  labs(title = "Mortality Rates by Region",
...
ggplot(income_group_data_196, aes(x = Year, y = Rate, color = `Geographic Value`)) +
  geom_line() +
  geom_point() +
  facet_wrap(~`Geographic Value`, scales = "free_y") +
...
ggplot(income_group_data_198, aes(x = Year, y = Rate, color = `Geographic Value`)) +
  geom_line() +
  geom_point() +
  facet_wrap(~`Geographic Value`, scales = "free_y") +
  labs(title = "Mortality Rates by World Bank Income Group",
...
```{r Predictors are Country, Year and Road User Types}

# RS_246 contains Distribution of road traffic deaths by type of road user (%)

# Ensure all potential levels are recognized for the 'Country' factor
all_countries <- unique(rs_246$Country)  # Get all unique country names

# Apply factor setting with all known levels to the 'Country' column
rs_246$Country <- factor(rs_246$Country, levels = all_countries)
...
# Model fitting
model <- lm(NumericValue ~ `Road User Type` + Year + Country, data = train_data)
...
# This step should now work without error
predictions <- predict(model, newdata = test_data)
rmse <- sqrt(mean((predictions - test_data$NumericValue, na.rm = TRUE)^2))
